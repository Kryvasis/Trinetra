#!/usr/bin/env bash
# V-128.sh — Prompt injection
# Category: Cloud, Client, Availability and LLM
# Tool: garak
# Usage: bash V-128.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-128.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-128.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-128.sh <session> <target>}"
...
CMD="garak --model_type rest --model_name $TARGET --probes promptinject"

echo "[hex] V-128: Prompt injection → $TARGET"
...
python3 - \
    ...
    "V-128" "Prompt injection" "Cloud, Client, Availability and LLM" "garak" \
