#!/usr/bin/env bash
# V-112.sh — I-PPE (Indirect Pipeline Poisoning)
# Category: Cloud, Client, Availability and LLM
# Tool: semgrep
# Usage: bash V-112.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-112.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-112.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-112.sh <session> <target>}"
...
CMD="semgrep --config=auto /tmp/target_src --json"

echo "[hex] V-112: I-PPE (Indirect Pipeline Poisoning) → $TARGET"
...
python3 - \
    ...
    "V-112" "I-PPE (Indirect Pipeline Poisoning)" "Cloud, Client, Availability and LLM" "semgrep" \
