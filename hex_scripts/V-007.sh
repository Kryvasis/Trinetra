#!/usr/bin/env bash
# V-007.sh — Weak Cipher suite
# Category: Discovery, Web and API
# Tool: sslscan
# Usage: bash V-007.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-007.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-007.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-007.sh <session> <target>}"
...
CMD="sslscan --no-colour --show-ciphers $TARGET"

echo "[hex] V-007: Weak Cipher suite → $TARGET"
...
python3 - \
    ...
    "V-007" "Weak Cipher suite" "Discovery, Web and API" "sslscan" \
