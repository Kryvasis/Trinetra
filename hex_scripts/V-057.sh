#!/usr/bin/env bash
# V-057.sh — Hardcoded secrets / API keys in source
# Category: Discovery, Web and API
# Tool: trufflehog
# Usage: bash V-057.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-057.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-057.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-057.sh <session> <target>}"
...
CMD="trufflehog filesystem --path=/tmp/target_src --only-verified"

echo "[hex] V-057: Hardcoded secrets / API keys in source → $TARGET"
...
python3 - \
    ...
    "V-057" "Hardcoded secrets / API keys in source" "Discovery, Web and API" "trufflehog" \
