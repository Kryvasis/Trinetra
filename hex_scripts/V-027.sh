#!/usr/bin/env bash
# V-027.sh — Vertical privilege escalation
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-027.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-027.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-027.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-027.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/privilege-escalation.yaml -silent"

echo "[hex] V-027: Vertical privilege escalation → $TARGET"
...
python3 - \
    ...
    "V-027" "Vertical privilege escalation" "Discovery, Web and API" "nuclei" \
