...
# Tool: nuclei
# Usage: bash V-024.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-024.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-024.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-024.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/bola.yaml -silent"

echo "[hex] V-024: BOLA → $TARGET"
...
python3 - \
    ...
    "V-024" "BOLA" "Discovery, Web and API" "nuclei" \
