#!/usr/bin/env bash
# V-020.sh — OAuth misconfiguration
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-020.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-020.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-020.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-020.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/oauth/ -silent"

echo "[hex] V-020: OAuth misconfiguration → $TARGET"
...
python3 - \
    ...
    "V-020" "OAuth misconfiguration" "Discovery, Web and API" "nuclei" \
