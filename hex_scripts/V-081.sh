#!/usr/bin/env bash
# V-081.sh — NPD (Null Pointer Dereference)
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-081.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-081.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-081.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-081.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/null-pointer.yaml -silent"

echo "[hex] V-081: NPD (Null Pointer Dereference) → $TARGET"
...
python3 - \
    ...
    "V-081" "NPD (Null Pointer Dereference)" "Host, Software and internal architecture" "nuclei" \
