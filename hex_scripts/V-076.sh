#!/usr/bin/env bash
# V-076.sh — Heap overflow
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-076.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-076.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-076.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-076.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/heap-overflow.yaml -silent"

echo "[hex] V-076: Heap overflow → $TARGET"
...
python3 - \
    ...
    "V-076" "Heap overflow" "Host, Software and internal architecture" "nuclei" \
