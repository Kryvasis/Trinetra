#!/usr/bin/env bash
# V-018.sh — Session fixation
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-018.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-018.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-018.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-018.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/session-fixation.yaml -silent"

echo "[hex] V-018: Session fixation → $TARGET"
...
python3 - \
    ...
    "V-018" "Session fixation" "Discovery, Web and API" "nuclei" \
