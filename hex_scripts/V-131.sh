#!/usr/bin/env bash
# V-131.sh — Data / training poisoning
# Category: Cloud, Client, Availability and LLM
# Tool: garak
# Usage: bash V-131.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-131.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-131.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-131.sh <session> <target>}"
...
CMD="garak --model_type rest --model_name $TARGET --probes poisoning"

echo "[hex] V-131: Data / training poisoning → $TARGET"
...
python3 - \
    ...
    "V-131" "Data / training poisoning" "Cloud, Client, Availability and LLM" "garak" \
