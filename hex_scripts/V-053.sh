#!/usr/bin/env bash
# V-053.sh — Business logic flaws
# Category: Discovery, Web and API
# Tool: ffuf
# Usage: bash V-053.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-053.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-053.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-053.sh <session> <target>}"
...
CMD="ffuf -u http://$TARGET/FUZZ -w /usr/share/seclists/Discovery/Web-Content/raft-large-words.txt -mc 200,201,204,301,302 -s"

echo "[hex] V-053: Business logic flaws → $TARGET"
...
python3 - \
    ...
    "V-053" "Business logic flaws" "Discovery, Web and API" "ffuf" \
