...
# Tool: netexec
# Usage: bash V-093.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-093.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-093.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-093.sh <session> <target>}"
...
echo "[hex] V-093: SMB relay → $TARGET"
...
python3 - \
    ...
    "V-093" "SMB relay" "Host, Software and internal architecture" "netexec" \
