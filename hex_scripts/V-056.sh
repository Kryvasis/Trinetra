#!/usr/bin/env bash
# V-056.sh — Insecure session token design
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-056.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-056.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-056.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-056.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/session-token-weak.yaml -silent"

echo "[hex] V-056: Insecure session token design → $TARGET"
...
python3 - \
    ...
    "V-056" "Insecure session token design" "Discovery, Web and API" "nuclei" \
