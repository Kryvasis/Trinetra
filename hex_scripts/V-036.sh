#!/usr/bin/env bash
# V-036.sh — DOM-Based XSS
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-036.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-036.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-036.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-036.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/xss/dom-based-xss.yaml -silent"

echo "[hex] V-036: DOM-Based XSS → $TARGET"
...
python3 - \
    ...
    "V-036" "DOM-Based XSS" "Discovery, Web and API" "nuclei" \
