#!/usr/bin/env bash
# V-013.sh — Weak password policy
# Category: Discovery, Web and API
# Tool: hydra
# Usage: bash V-013.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-013.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-013.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-013.sh <session> <target>}"
...
CMD="hydra -l admin -P /usr/share/wordlists/rockyou.txt $TARGET http-post-form '/login:user=^USER^&pass=^PASS^:F=incorrect' -f -q"

echo "[hex] V-013: Weak password policy → $TARGET"
...
python3 - \
    ...
    "V-013" "Weak password policy" "Discovery, Web and API" "hydra" \
