#!/usr/bin/env bash
# V-017.sh — Insecure password reset flow
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-017.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-017.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-017.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-017.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/password-reset-poisoning.yaml -silent"

echo "[hex] V-017: Insecure password reset flow → $TARGET"
...
python3 - \
    ...
    "V-017" "Insecure password reset flow" "Discovery, Web and API" "nuclei" \
