#!/usr/bin/env bash
# V-009.sh — Lack of SSL pinning
# Category: Discovery, Web and API
# Tool: sslscan
# Usage: bash V-009.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-009.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-009.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-009.sh <session> <target>}"
...
CMD="sslscan --no-colour --show-certificate $TARGET"

echo "[hex] V-009: Lack of SSL pinning → $TARGET"
...
python3 - \
    ...
    "V-009" "Lack of SSL pinning" "Discovery, Web and API" "sslscan" \
