#!/usr/bin/env bash
# V-028.sh — Horizontal privilege escalation
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-028.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-028.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-028.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-028.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ -tags privesc -silent"

echo "[hex] V-028: Horizontal privilege escalation → $TARGET"
...
python3 - \
    ...
    "V-028" "Horizontal privilege escalation" "Discovery, Web and API" "nuclei" \
