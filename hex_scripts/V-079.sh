#!/usr/bin/env bash
# V-079.sh — OOBR (Out-of-Bounds Read)
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-079.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-079.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-079.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-079.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/oob-read.yaml -silent"

echo "[hex] V-079: OOBR (Out-of-Bounds Read) → $TARGET"
...
python3 - \
    ...
    "V-079" "OOBR (Out-of-Bounds Read)" "Host, Software and internal architecture" "nuclei" \
