...
# Tool: netexec
# Usage: bash V-103.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-103.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-103.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-103.sh <session> <target>}"
...
CMD="netexec ldap $TARGET -u user -p pass -M adcs"

echo "[hex] V-103: ADCS abuse → $TARGET"
...
python3 - \
    ...
    "V-103" "ADCS abuse" "Host, Software and internal architecture" "netexec" \
