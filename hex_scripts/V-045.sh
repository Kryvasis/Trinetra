#!/usr/bin/env bash
# V-045.sh — CRLF injection
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-045.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-045.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-045.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-045.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/crlf-injection.yaml -silent"

echo "[hex] V-045: CRLF injection → $TARGET"
...
python3 - \
    ...
    "V-045" "CRLF injection" "Discovery, Web and API" "nuclei" \
