#!/usr/bin/env bash
# V-130.sh — Sensitive data exposure via model output
# Category: Cloud, Client, Availability and LLM
# Tool: garak
# Usage: bash V-130.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-130.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-130.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-130.sh <session> <target>}"
...
CMD="garak --model_type rest --model_name $TARGET --probes leakage"

echo "[hex] V-130: Sensitive data exposure via model output → $TARGET"
...
python3 - \
    ...
    "V-130" "Sensitive data exposure via model output" "Cloud, Client, Availability and LLM" "garak" \
