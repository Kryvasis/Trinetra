...
# Tool: nuclei
# Usage: bash V-068.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-068.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-068.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-068.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/rce.yaml -silent"

echo "[hex] V-068: RCE → $TARGET"
...
python3 - \
    ...
    "V-068" "RCE" "Host, Software and internal architecture" "nuclei" \
