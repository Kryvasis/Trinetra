#!/usr/bin/env bash
# V-129.sh — LLM jailbreak
# Category: Cloud, Client, Availability and LLM
# Tool: garak
# Usage: bash V-129.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-129.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-129.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-129.sh <session> <target>}"
...
CMD="garak --model_type rest --model_name $TARGET --probes dan"

echo "[hex] V-129: LLM jailbreak → $TARGET"
...
python3 - \
    ...
    "V-129" "LLM jailbreak" "Cloud, Client, Availability and LLM" "garak" \
