#!/usr/bin/env bash
# V-116.sh — UAC bypassing
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-116.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-116.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-116.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-116.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/uac-bypass.yaml -silent"

echo "[hex] V-116: UAC bypassing → $TARGET"
...
python3 - \
    ...
    "V-116" "UAC bypassing" "Cloud, Client, Availability and LLM" "nuclei" \
