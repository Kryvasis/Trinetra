...
# Tool: nuclei
# Usage: bash V-025.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-025.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-025.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-025.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/bfla.yaml -silent"

echo "[hex] V-025: BFLA → $TARGET"
...
python3 - \
    ...
    "V-025" "BFLA" "Discovery, Web and API" "nuclei" \
