...
# Tool: nuclei
# Usage: bash V-052.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-052.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-052.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-052.sh <session> <target>}"
...
python3 - \
    ...
    "V-052" "ReDoS" "Discovery, Web and API" "nuclei" \
