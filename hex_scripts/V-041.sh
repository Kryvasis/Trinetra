...
# Tool: nuclei
# Usage: bash V-041.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-041.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-041.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-041.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/esi-injection.yaml -silent"

echo "[hex] V-041: ESI → $TARGET"
...
python3 - \
    ...
    "V-041" "ESI" "Discovery, Web and API" "nuclei" \
