#!/usr/bin/env bash
# V-004.sh — DNS spoofing
# Category: Discovery, Web and API
# Tool: dnsenum
# Usage: bash V-004.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-004.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-004.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-004.sh <session> <target>}"
...
CMD="dnsenum --dnsserver 8.8.8.8 --enum $TARGET"

echo "[hex] V-004: DNS spoofing → $TARGET"
...
python3 - \
    ...
    "V-004" "DNS spoofing" "Discovery, Web and API" "dnsenum" \
