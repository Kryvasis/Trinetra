...
# Tool: nuclei
# Usage: bash V-125.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-125.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-125.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-125.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ddos-amplification.yaml -silent"

echo "[hex] V-125: DDoS → $TARGET"
...
python3 - \
    ...
    "V-125" "DDoS" "Cloud, Client, Availability and LLM" "nuclei" \
