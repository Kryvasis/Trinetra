...
# Tool: nuclei
# Usage: bash V-023.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-023.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-023.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-023.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/idor.yaml -silent"

echo "[hex] V-023: IDOR → $TARGET"
...
python3 - \
    ...
    "V-023" "IDOR" "Discovery, Web and API" "nuclei" \
