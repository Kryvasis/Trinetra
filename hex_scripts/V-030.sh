...
# Tool: nuclei
# Usage: bash V-030.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-030.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-030.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-030.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/nosql-injection.yaml -silent"

echo "[hex] V-030: NoSQLi → $TARGET"
...
python3 - \
    ...
    "V-030" "NoSQLi" "Discovery, Web and API" "nuclei" \
