#!/usr/bin/env bash
# V-106.sh — Public accessible cloud storage exposure
# Category: Cloud, Client, Availability and LLM
# Tool: cloud_enum
# Usage: bash V-106.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-106.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-106.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-106.sh <session> <target>}"
...
CMD="cloud_enum -k $TARGET"

echo "[hex] V-106: Public accessible cloud storage exposure → $TARGET"
...
python3 - \
    ...
    "V-106" "Public accessible cloud storage exposure" "Cloud, Client, Availability and LLM" "cloud_enum" \
