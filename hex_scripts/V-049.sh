...
# Tool: nuclei
# Usage: bash V-049.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-049.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-049.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-049.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/http-request-smuggling.yaml -silent"

echo "[hex] V-049: HRS / DESYNC → $TARGET"
...
python3 - \
    ...
    "V-049" "HRS / DESYNC" "Discovery, Web and API" "nuclei" \
