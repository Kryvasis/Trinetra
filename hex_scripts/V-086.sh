#!/usr/bin/env bash
# V-086.sh — ROP exposure
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-086.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-086.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-086.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-086.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/rop.yaml -silent"

echo "[hex] V-086: ROP exposure → $TARGET"
...
python3 - \
    ...
    "V-086" "ROP exposure" "Host, Software and internal architecture" "nuclei" \
