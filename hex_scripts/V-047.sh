...
# Tool: nuclei
# Usage: bash V-047.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-047.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-047.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-047.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ssrf.yaml -silent"

echo "[hex] V-047: SSRF → $TARGET"
...
python3 - \
    ...
    "V-047" "SSRF" "Discovery, Web and API" "nuclei" \
