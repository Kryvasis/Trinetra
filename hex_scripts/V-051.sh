...
# Tool: nuclei
# Usage: bash V-051.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-051.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-051.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-051.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/waf-bypass/ -silent"

echo "[hex] V-051: WAF bypass → $TARGET"
...
python3 - \
    ...
    "V-051" "WAF bypass" "Discovery, Web and API" "nuclei" \
