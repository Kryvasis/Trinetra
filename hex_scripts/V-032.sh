...
# Tool: nuclei
# Usage: bash V-032.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-032.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-032.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-032.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/xpath-injection.yaml -silent"

echo "[hex] V-032: XPATHi → $TARGET"
...
python3 - \
    ...
    "V-032" "XPATHi" "Discovery, Web and API" "nuclei" \
