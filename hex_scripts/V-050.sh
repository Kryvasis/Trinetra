#!/usr/bin/env bash
# V-050.sh — CORS misconfiguration
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-050.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-050.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-050.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-050.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/cors-misconfiguration.yaml -silent"

echo "[hex] V-050: CORS misconfiguration → $TARGET"
...
python3 - \
    ...
    "V-050" "CORS misconfiguration" "Discovery, Web and API" "nuclei" \
