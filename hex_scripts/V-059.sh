#!/usr/bin/env bash
# V-059.sh — Unencrypted sensitive data in local storage
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-059.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-059.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-059.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-059.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/insecure-storage.yaml -silent"

echo "[hex] V-059: Unencrypted sensitive data in local storage → $TARGET"
...
python3 - \
    ...
    "V-059" "Unencrypted sensitive data in local storage" "Discovery, Web and API" "nuclei" \
