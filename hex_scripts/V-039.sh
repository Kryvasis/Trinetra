...
# Tool: nuclei
# Usage: bash V-039.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-039.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-039.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-039.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/csti.yaml -silent"

echo "[hex] V-039: CSTI → $TARGET"
...
python3 - \
    ...
    "V-039" "CSTI" "Discovery, Web and API" "nuclei" \
