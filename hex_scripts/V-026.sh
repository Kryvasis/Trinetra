...
# Tool: nuclei
# Usage: bash V-026.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-026.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-026.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-026.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ -tags bopla -silent"

echo "[hex] V-026: BOPLA → $TARGET"
...
python3 - \
    ...
    "V-026" "BOPLA" "Discovery, Web and API" "nuclei" \
