...
# Tool: nuclei
# Usage: bash V-031.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-031.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-031.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-031.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ldap-injection.yaml -silent"

echo "[hex] V-031: LDAPi → $TARGET"
...
python3 - \
    ...
    "V-031" "LDAPi" "Discovery, Web and API" "nuclei" \
