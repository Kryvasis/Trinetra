#!/usr/bin/env bash
# V-126.sh — ReDoS / algorithmic complexity DoS
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-126.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-126.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-126.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-126.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/redos.yaml -silent"

echo "[hex] V-126: ReDoS / algorithmic complexity DoS → $TARGET"
...
python3 - \
    ...
    "V-126" "ReDoS / algorithmic complexity DoS" "Cloud, Client, Availability and LLM" "nuclei" \
