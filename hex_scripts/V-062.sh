...
# Tool: nuclei
# Usage: bash V-062.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-062.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-062.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-062.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/lfi.yaml -silent"

echo "[hex] V-062: LFI → $TARGET"
...
python3 - \
    ...
    "V-062" "LFI" "Discovery, Web and API" "nuclei" \
