#!/usr/bin/env bash
# V-080.sh — OOBW (Out-of-Bounds Write)
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-080.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-080.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-080.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-080.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/oob-write.yaml -silent"

echo "[hex] V-080: OOBW (Out-of-Bounds Write) → $TARGET"
...
python3 - \
    ...
    "V-080" "OOBW (Out-of-Bounds Write)" "Host, Software and internal architecture" "nuclei" \
