...
# Tool: dalfox
# Usage: bash V-035.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-035.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-035.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-035.sh <session> <target>}"
...
CMD="dalfox url http://$TARGET --silence --no-spinner --stored"

echo "[hex] V-035: Stored XSS → $TARGET"
...
python3 - \
    ...
    "V-035" "Stored XSS" "Discovery, Web and API" "dalfox" \
