#!/usr/bin/env bash
# V-021.sh — SAML bypass / signature wrapping
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-021.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-021.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-021.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-021.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/saml-injection.yaml -silent"

echo "[hex] V-021: SAML bypass / signature wrapping → $TARGET"
...
python3 - \
    ...
    "V-021" "SAML bypass / signature wrapping" "Discovery, Web and API" "nuclei" \
