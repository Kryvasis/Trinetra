...
# Tool: nuclei
# Usage: bash V-040.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-040.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-040.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-040.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ssi-injection.yaml -silent"

echo "[hex] V-040: SSI → $TARGET"
...
python3 - \
    ...
    "V-040" "SSI" "Discovery, Web and API" "nuclei" \
