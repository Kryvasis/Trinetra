#!/usr/bin/env bash
# V-105.sh — Flat network / missing segmentation
# Category: Host, Software and internal architecture
# Tool: nmap
# Usage: bash V-105.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-105.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-105.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-105.sh <session> <target>}"
...
CMD="nmap -sn $TARGET/24 --open"

echo "[hex] V-105: Flat network / missing segmentation → $TARGET"
...
python3 - \
    ...
    "V-105" "Flat network / missing segmentation" "Host, Software and internal architecture" "nmap" \
