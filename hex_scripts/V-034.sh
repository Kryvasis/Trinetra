#!/usr/bin/env bash
# V-034.sh — Reflected XSS
# Category: Discovery, Web and API
# Tool: dalfox
# Usage: bash V-034.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-034.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-034.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-034.sh <session> <target>}"
...
CMD="dalfox url http://$TARGET --silence --no-spinner"

echo "[hex] V-034: Reflected XSS → $TARGET"
...
python3 - \
    ...
    "V-034" "Reflected XSS" "Discovery, Web and API" "dalfox" \
