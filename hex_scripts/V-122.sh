#!/usr/bin/env bash
# V-122.sh — Insecure local storage in client app
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-122.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-122.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-122.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-122.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/insecure-local-storage.yaml -silent"

echo "[hex] V-122: Insecure local storage in client app → $TARGET"
...
python3 - \
    ...
    "V-122" "Insecure local storage in client app" "Cloud, Client, Availability and LLM" "nuclei" \
