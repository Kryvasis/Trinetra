#!/usr/bin/env bash
# V-003.sh — Open port and unnecessary service exposure
# Category: Discovery, Web and API
# Tool: nmap
# Usage: bash V-003.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-003.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-003.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-003.sh <session> <target>}"
...
CMD="nmap -sV -T4 -p- --open $TARGET"

echo "[hex] V-003: Open port and unnecessary service exposure → $TARGET"
...
python3 - \
    ...
    "V-003" "Open port and unnecessary service exposure" "Discovery, Web and API" "nmap" \
