...
# Tool: nuclei
# Usage: bash V-038.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-038.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-038.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-038.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/ssti.yaml -silent"

echo "[hex] V-038: SSTI → $TARGET"
...
python3 - \
    ...
    "V-038" "SSTI" "Discovery, Web and API" "nuclei" \
