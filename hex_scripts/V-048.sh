#!/usr/bin/env bash
# V-048.sh — Open redirect
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-048.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-048.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-048.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-048.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/open-redirect.yaml -silent"

echo "[hex] V-048: Open redirect → $TARGET"
...
python3 - \
    ...
    "V-048" "Open redirect" "Discovery, Web and API" "nuclei" \
