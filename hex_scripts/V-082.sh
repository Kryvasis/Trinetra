#!/usr/bin/env bash
# V-082.sh — Memory leak
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-082.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-082.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-082.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-082.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/memory-leak.yaml -silent"

echo "[hex] V-082: Memory leak → $TARGET"
...
python3 - \
    ...
    "V-082" "Memory leak" "Host, Software and internal architecture" "nuclei" \
