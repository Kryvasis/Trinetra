#!/usr/bin/env bash
# V-077.sh — UAF (Use-After-Free)
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-077.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-077.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-077.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-077.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/uaf.yaml -silent"

echo "[hex] V-077: UAF (Use-After-Free) → $TARGET"
...
python3 - \
    ...
    "V-077" "UAF (Use-After-Free)" "Host, Software and internal architecture" "nuclei" \
