#!/usr/bin/env bash
# V-078.sh — Double free
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-078.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-078.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-078.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-078.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/double-free.yaml -silent"

echo "[hex] V-078: Double free → $TARGET"
...
python3 - \
    ...
    "V-078" "Double free" "Host, Software and internal architecture" "nuclei" \
