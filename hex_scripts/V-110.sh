...
# Tool: nuclei
# Usage: bash V-110.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-110.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-110.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-110.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/ -tags cloud -silent"

echo "[hex] V-110: CSPM flaw → $TARGET"
...
python3 - \
    ...
    "V-110" "CSPM flaw" "Cloud, Client, Availability and LLM" "nuclei" \
