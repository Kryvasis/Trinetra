#!/usr/bin/env bash
# V-019.sh — Weak JWT validation and signature bypass
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-019.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-019.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-019.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-019.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/jwt-none-algorithm.yaml -t vulnerabilities/generic/jwt-weak-secret.yaml -silent"

echo "[hex] V-019: Weak JWT validation and signature bypass → $TARGET"
...
python3 - \
    ...
    "V-019" "Weak JWT validation and signature bypass" "Discovery, Web and API" "nuclei" \
