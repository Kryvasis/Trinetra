...
# Tool: nuclei
# Usage: bash V-046.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-046.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-046.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-046.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/csrf.yaml -silent"

echo "[hex] V-046: CSRF → $TARGET"
...
python3 - \
    ...
    "V-046" "CSRF" "Discovery, Web and API" "nuclei" \
