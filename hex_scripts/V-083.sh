#!/usr/bin/env bash
# V-083.sh — FSV / FMT (Format String)
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-083.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-083.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-083.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-083.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/format-string.yaml -silent"

echo "[hex] V-083: FSV / FMT (Format String) → $TARGET"
...
python3 - \
    ...
    "V-083" "FSV / FMT (Format String)" "Host, Software and internal architecture" "nuclei" \
