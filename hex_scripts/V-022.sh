...
# Tool: ffuf
# Usage: bash V-022.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-022.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-022.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-022.sh <session> <target>}"
...
CMD="ffuf -u http://$TARGET/FUZZ -w /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt -mc 200,403 -s"

echo "[hex] V-022: BAC → $TARGET"
...
python3 - \
    ...
    "V-022" "BAC" "Discovery, Web and API" "ffuf" \
