#!/usr/bin/env bash
# V-084.sh — Race condition
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-084.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-084.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-084.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-084.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/race-condition.yaml -silent"

echo "[hex] V-084: Race condition → $TARGET"
...
python3 - \
    ...
    "V-084" "Race condition" "Host, Software and internal architecture" "nuclei" \
