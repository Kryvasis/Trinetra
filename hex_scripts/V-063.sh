...
# Tool: nuclei
# Usage: bash V-063.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-063.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-063.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-063.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/rfi.yaml -silent"

echo "[hex] V-063: RFI → $TARGET"
...
python3 - \
    ...
    "V-063" "RFI" "Discovery, Web and API" "nuclei" \
