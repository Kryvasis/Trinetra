#!/usr/bin/env bash
# V-006.sh — Weak TLS/SSL version
# Category: Discovery, Web and API
# Tool: sslscan
# Usage: bash V-006.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-006.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-006.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-006.sh <session> <target>}"
...
echo "[hex] V-006: Weak TLS/SSL version → $TARGET"
...
python3 - \
    ...
    "V-006" "Weak TLS/SSL version" "Discovery, Web and API" "sslscan" \
