#!/usr/bin/env bash
# V-008.sh — Expired or self signed certificate
# Category: Discovery, Web and API
# Tool: openssl
# Usage: bash V-008.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-008.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-008.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-008.sh <session> <target>}"
...
CMD="openssl s_client -connect $TARGET:443 -showcerts </dev/null 2>/dev/null"

echo "[hex] V-008: Expired or self signed certificate → $TARGET"
...
python3 - \
    ...
    "V-008" "Expired or self signed certificate" "Discovery, Web and API" "openssl" \
