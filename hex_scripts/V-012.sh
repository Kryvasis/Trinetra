#!/usr/bin/env bash
# V-012.sh — Default credentials
# Category: Discovery, Web and API
# Tool: hydra
# Usage: bash V-012.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-012.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-012.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-012.sh <session> <target>}"
...
CMD="hydra -L /usr/share/seclists/Passwords/Default-Credentials/default-usernames.txt -P /usr/share/seclists/Passwords/Default-Credentials/default-passwords.txt $TARGET http-post-form '/login:user=^USER^&pass=^PASS^:F=incorrect' -f -q"

echo "[hex] V-012: Default credentials → $TARGET"
...
python3 - \
    ...
    "V-012" "Default credentials" "Discovery, Web and API" "hydra" \
