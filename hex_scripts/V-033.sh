...
# Tool: nuclei
# Usage: bash V-033.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-033.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-033.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-033.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/command-injection.yaml -silent"

echo "[hex] V-033: CMDi / OSCi → $TARGET"
...
python3 - \
    ...
    "V-033" "CMDi / OSCi" "Discovery, Web and API" "nuclei" \
