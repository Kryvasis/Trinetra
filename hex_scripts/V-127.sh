#!/usr/bin/env bash
# V-127.sh — Missing rate limiting / Brute force control
# Category: Cloud, Client, Availability and LLM
# Tool: ffuf
# Usage: bash V-127.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-127.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-127.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-127.sh <session> <target>}"
...
CMD="ffuf -u http://$TARGET/login -X POST -d 'user=admin&pass=FUZZ' -w /usr/share/seclists/Passwords/Common-Credentials/10k-most-common.txt -mc 200 -s"

echo "[hex] V-127: Missing rate limiting / Brute force control → $TARGET"
...
python3 - \
    ...
    "V-127" "Missing rate limiting / Brute force control" "Cloud, Client, Availability and LLM" "ffuf" \
