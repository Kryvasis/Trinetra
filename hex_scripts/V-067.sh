#!/usr/bin/env bash
# V-067.sh — Path / directory traversal
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-067.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-067.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-067.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-067.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/path-traversal.yaml -silent"

echo "[hex] V-067: Path / directory traversal → $TARGET"
...
python3 - \
    ...
    "V-067" "Path / directory traversal" "Discovery, Web and API" "nuclei" \
