...
# Tool: nuclei
# Usage: bash V-072.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-072.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-072.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-072.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/vm-escape.yaml -silent"

echo "[hex] V-072: VM escape → $TARGET"
...
python3 - \
    ...
    "V-072" "VM escape" "Host, Software and internal architecture" "nuclei" \
