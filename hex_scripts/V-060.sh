#!/usr/bin/env bash
# V-060.sh — GPP password exposure
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-060.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-060.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-060.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-060.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t exposures/ -tags gpp -silent"

echo "[hex] V-060: GPP password exposure → $TARGET"
...
python3 - \
    ...
    "V-060" "GPP password exposure" "Discovery, Web and API" "nuclei" \
