#!/usr/bin/env bash
# V-073.sh — Misconfigured file/directory permissions
# Category: Host, Software and internal architecture
# Tool: nmap
# Usage: bash V-073.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-073.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-073.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-073.sh <session> <target>}"
...
CMD="nmap -sV --script ftp-anon,smb-enum-shares $TARGET"

echo "[hex] V-073: Misconfigured file/directory permissions → $TARGET"
...
python3 - \
    ...
    "V-073" "Misconfigured file/directory permissions" "Host, Software and internal architecture" "nmap" \
