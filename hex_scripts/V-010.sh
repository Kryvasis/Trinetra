#!/usr/bin/env bash
# V-010.sh — Missing HSTS header
# Category: Discovery, Web and API
# Tool: curl
# Usage: bash V-010.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-010.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-010.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-010.sh <session> <target>}"
...
CMD="curl -sI https://$TARGET"

echo "[hex] V-010: Missing HSTS header → $TARGET"
...
python3 - \
    ...
    "V-010" "Missing HSTS header" "Discovery, Web and API" "curl" \
