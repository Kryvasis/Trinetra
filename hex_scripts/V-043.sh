...
# Tool: nuclei
# Usage: bash V-043.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-043.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-043.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-043.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/host-header-injection.yaml -silent"

echo "[hex] V-043: HHI → $TARGET"
...
python3 - \
    ...
    "V-043" "HHI" "Discovery, Web and API" "nuclei" \
