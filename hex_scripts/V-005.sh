#!/usr/bin/env bash
# V-005.sh — Banner grabbing and fingerprinting
# Category: Discovery, Web and API
# Tool: whatweb
# Usage: bash V-005.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-005.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-005.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-005.sh <session> <target>}"
...
echo "[hex] V-005: Banner grabbing and fingerprinting → $TARGET"
...
python3 - \
    ...
    "V-005" "Banner grabbing and fingerprinting" "Discovery, Web and API" "whatweb" \
