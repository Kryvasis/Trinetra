#!/usr/bin/env bash
# V-075.sh — Stack buffer overflow
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-075.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-075.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-075.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-075.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/buffer-overflow.yaml -silent"

echo "[hex] V-075: Stack buffer overflow → $TARGET"
...
python3 - \
    ...
    "V-075" "Stack buffer overflow" "Host, Software and internal architecture" "nuclei" \
