...
# Tool: nuclei
# Usage: bash V-124.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-124.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-124.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-124.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/dos.yaml -silent"

echo "[hex] V-124: DoS → $TARGET"
...
python3 - \
    ...
    "V-124" "DoS" "Cloud, Client, Availability and LLM" "nuclei" \
