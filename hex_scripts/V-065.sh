#!/usr/bin/env bash
# V-065.sh — Insecure deserialization
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-065.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-065.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-065.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-065.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/insecure-deserialization.yaml -silent"

echo "[hex] V-065: Insecure deserialization → $TARGET"
...
python3 - \
    ...
    "V-065" "Insecure deserialization" "Discovery, Web and API" "nuclei" \
