...
# Tool: nuclei
# Usage: bash V-037.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-037.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-037.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-037.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/xxe-injection.yaml -silent"

echo "[hex] V-037: XXE → $TARGET"
...
python3 - \
    ...
    "V-037" "XXE" "Discovery, Web and API" "nuclei" \
