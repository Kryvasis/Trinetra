...
# Tool: nuclei
# Usage: bash V-015.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-015.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-015.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-015.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/account-takeover.yaml -silent"

echo "[hex] V-015: ATO → $TARGET"
...
python3 - \
    ...
    "V-015" "ATO" "Discovery, Web and API" "nuclei" \
