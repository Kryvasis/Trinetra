...
# Tool: nuclei
# Usage: bash V-069.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-069.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-069.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-069.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/lpe.yaml -silent"

echo "[hex] V-069: LPE → $TARGET"
...
python3 - \
    ...
    "V-069" "LPE" "Host, Software and internal architecture" "nuclei" \
