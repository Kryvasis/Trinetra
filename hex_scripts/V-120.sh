#!/usr/bin/env bash
# V-120.sh — Deep link hijacking
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-120.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-120.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-120.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-120.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/deeplink-hijack.yaml -silent"

echo "[hex] V-120: Deep link hijacking → $TARGET"
...
python3 - \
    ...
    "V-120" "Deep link hijacking" "Cloud, Client, Availability and LLM" "nuclei" \
