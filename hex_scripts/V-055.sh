#!/usr/bin/env bash
# V-055.sh — Insecure GraphQL introspection / batching
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-055.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-055.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-055.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-055.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/graphql/ -silent"

echo "[hex] V-055: Insecure GraphQL introspection / batching → $TARGET"
...
python3 - \
    ...
    "V-055" "Insecure GraphQL introspection / batching" "Discovery, Web and API" "nuclei" \
