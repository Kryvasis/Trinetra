...
# Tool: curl
# Usage: bash V-109.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-109.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-109.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-109.sh <session> <target>}"
...
echo "[hex] V-109: IMDS abuse → $TARGET"
...
python3 - \
    ...
    "V-109" "IMDS abuse" "Cloud, Client, Availability and LLM" "curl" \
