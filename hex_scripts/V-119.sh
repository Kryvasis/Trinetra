#!/usr/bin/env bash
# V-119.sh — Insecure IPC / exported component abuse
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-119.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-119.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-119.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-119.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/insecure-ipc.yaml -silent"

echo "[hex] V-119: Insecure IPC / exported component abuse → $TARGET"
...
python3 - \
    ...
    "V-119" "Insecure IPC / exported component abuse" "Cloud, Client, Availability and LLM" "nuclei" \
