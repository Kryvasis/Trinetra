#!/usr/bin/env bash
# V-091.sh — NBT-NS Poisoning
# Category: Host, Software and internal architecture
# Tool: responder
# Usage: bash V-091.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-091.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-091.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-091.sh <session> <target>}"
...
echo "[hex] V-091: NBT-NS Poisoning → $TARGET"
...
python3 - \
    ...
    "V-091" "NBT-NS Poisoning" "Host, Software and internal architecture" "responder" \
