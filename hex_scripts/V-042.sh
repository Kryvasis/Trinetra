#!/usr/bin/env bash
# V-042.sh — XSLT injection
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-042.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-042.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-042.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-042.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/xslt-injection.yaml -silent"

echo "[hex] V-042: XSLT injection → $TARGET"
...
python3 - \
    ...
    "V-042" "XSLT injection" "Discovery, Web and API" "nuclei" \
