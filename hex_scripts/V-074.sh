#!/usr/bin/env bash
# V-074.sh — Insecure cron job / scheduled task
# Category: Host, Software and internal architecture
# Tool: nuclei
# Usage: bash V-074.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-074.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-074.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-074.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/ -tags cron -silent"

echo "[hex] V-074: Insecure cron job / scheduled task → $TARGET"
...
python3 - \
    ...
    "V-074" "Insecure cron job / scheduled task" "Host, Software and internal architecture" "nuclei" \
