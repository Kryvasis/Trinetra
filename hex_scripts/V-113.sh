#!/usr/bin/env bash
# V-113.sh — Build secret exposure
# Category: Cloud, Client, Availability and LLM
# Tool: trufflehog
# Usage: bash V-113.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-113.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-113.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-113.sh <session> <target>}"
...
CMD="trufflehog git file:///tmp/target_src --only-verified"

echo "[hex] V-113: Build secret exposure → $TARGET"
...
python3 - \
    ...
    "V-113" "Build secret exposure" "Cloud, Client, Availability and LLM" "trufflehog" \
