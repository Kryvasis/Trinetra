...
# Tool: nuclei
# Usage: bash V-100.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-100.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-100.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-100.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/dcshadow.yaml -silent"

echo "[hex] V-100: DCShadow → $TARGET"
...
python3 - \
    ...
    "V-100" "DCShadow" "Host, Software and internal architecture" "nuclei" \
