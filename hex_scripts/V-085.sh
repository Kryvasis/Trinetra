...
# Tool: nuclei
# Usage: bash V-085.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-085.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-085.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-085.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/toctou.yaml -silent"

echo "[hex] V-085: TOCTOU → $TARGET"
...
python3 - \
    ...
    "V-085" "TOCTOU" "Host, Software and internal architecture" "nuclei" \
