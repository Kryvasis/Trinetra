#!/usr/bin/env bash
# V-016.sh — Credential stuffing exposure
# Category: Discovery, Web and API
# Tool: hydra
# Usage: bash V-016.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-016.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-016.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-016.sh <session> <target>}"
...
CMD="hydra -L /usr/share/seclists/Usernames/top-usernames-shortlist.txt -P /usr/share/seclists/Passwords/Common-Credentials/10k-most-common.txt $TARGET http-post-form '/login:user=^USER^&pass=^PASS^:F=incorrect' -f -q"

echo "[hex] V-016: Credential stuffing exposure → $TARGET"
...
python3 - \
    ...
    "V-016" "Credential stuffing exposure" "Discovery, Web and API" "hydra" \
