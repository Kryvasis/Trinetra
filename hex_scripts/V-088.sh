#!/usr/bin/env bash
# V-088.sh — Missing or incomplete SBOM
# Category: Host, Software and internal architecture
# Tool: syft
# Usage: bash V-088.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-088.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-088.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-088.sh <session> <target>}"
...
CMD="syft dir:/tmp/target_src -o spdx-json"

echo "[hex] V-088: Missing or incomplete SBOM → $TARGET"
...
python3 - \
    ...
    "V-088" "Missing or incomplete SBOM" "Host, Software and internal architecture" "syft" \
