#!/usr/bin/env bash
# V-087.sh — Vulnerable dependencies / SCA findings
# Category: Host, Software and internal architecture
# Tool: grype
# Usage: bash V-087.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-087.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-087.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-087.sh <session> <target>}"
...
CMD="grype dir:/tmp/target_src --output json"

echo "[hex] V-087: Vulnerable dependencies / SCA findings → $TARGET"
...
python3 - \
    ...
    "V-087" "Vulnerable dependencies / SCA findings" "Host, Software and internal architecture" "grype" \
