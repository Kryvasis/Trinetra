#!/usr/bin/env bash
# V-089.sh — ARP Spoofing
# Category: Host, Software and internal architecture
# Tool: nmap
# Usage: bash V-089.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-089.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-089.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-089.sh <session> <target>}"
...
CMD="nmap -PR -sn $TARGET/24"

echo "[hex] V-089: ARP Spoofing → $TARGET"
...
python3 - \
    ...
    "V-089" "ARP Spoofing" "Host, Software and internal architecture" "nmap" \
