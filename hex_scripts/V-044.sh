...
# Tool: nuclei
# Usage: bash V-044.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-044.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-044.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-044.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/http-parameter-pollution.yaml -silent"

echo "[hex] V-044: HPP → $TARGET"
...
python3 - \
    ...
    "V-044" "HPP" "Discovery, Web and API" "nuclei" \
