#!/usr/bin/env bash
# V-064.sh — Unrestricted / malicious file upload
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-064.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-064.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-064.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-064.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/unrestricted-file-upload.yaml -silent"

echo "[hex] V-064: Unrestricted / malicious file upload → $TARGET"
...
python3 - \
    ...
    "V-064" "Unrestricted / malicious file upload" "Discovery, Web and API" "nuclei" \
