#!/usr/bin/env bash
# V-054.sh — Mass assignment vulnerability
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-054.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-054.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-054.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-054.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/mass-assignment.yaml -silent"

echo "[hex] V-054: Mass assignment vulnerability → $TARGET"
...
python3 - \
    ...
    "V-054" "Mass assignment vulnerability" "Discovery, Web and API" "nuclei" \
