#!/usr/bin/env bash
# V-014.sh — Missing MFA / MFA bypass
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-014.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-014.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-014.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-014.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t exposures/tokens/ -t misconfiguration/ -silent"

echo "[hex] V-014: Missing MFA / MFA bypass → $TARGET"
...
python3 - \
    ...
    "V-014" "Missing MFA / MFA bypass" "Discovery, Web and API" "nuclei" \
