#!/usr/bin/env bash
# V-104.sh — VLAN hopping
# Category: Host, Software and internal architecture
# Tool: nmap
# Usage: bash V-104.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-104.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-104.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-104.sh <session> <target>}"
...
CMD="nmap -sV --script broadcast-dhcp-discover $TARGET"

echo "[hex] V-104: VLAN hopping → $TARGET"
...
python3 - \
    ...
    "V-104" "VLAN hopping" "Host, Software and internal architecture" "nmap" \
