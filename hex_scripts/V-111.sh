#!/usr/bin/env bash
# V-111.sh — D-PPE (Direct Pipeline Poisoning)
# Category: Cloud, Client, Availability and LLM
# Tool: semgrep
# Usage: bash V-111.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-111.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-111.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-111.sh <session> <target>}"
...
echo "[hex] V-111: D-PPE (Direct Pipeline Poisoning) → $TARGET"
...
python3 - \
    ...
    "V-111" "D-PPE (Direct Pipeline Poisoning)" "Cloud, Client, Availability and LLM" "semgrep" \
