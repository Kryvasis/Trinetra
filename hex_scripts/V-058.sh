#!/usr/bin/env bash
# V-058.sh — Sensitive data in logs / error messages
# Category: Discovery, Web and API
# Tool: ffuf
# Usage: bash V-058.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-058.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-058.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-058.sh <session> <target>}"
...
CMD="ffuf -u http://$TARGET/FUZZ -w /usr/share/seclists/Discovery/Web-Content/common.txt -mc 200 -s"

echo "[hex] V-058: Sensitive data in logs / error messages → $TARGET"
...
python3 - \
    ...
    "V-058" "Sensitive data in logs / error messages" "Discovery, Web and API" "ffuf" \
