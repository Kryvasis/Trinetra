#!/usr/bin/env bash
# V-066.sh — PHP object injection
# Category: Discovery, Web and API
# Tool: nuclei
# Usage: bash V-066.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-066.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-066.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-066.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/php/ -silent"

echo "[hex] V-066: PHP object injection → $TARGET"
...
python3 - \
    ...
    "V-066" "PHP object injection" "Discovery, Web and API" "nuclei" \
