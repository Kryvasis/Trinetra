#!/usr/bin/env bash
# V-115.sh — DLL hijacking
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-115.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-115.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-115.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-115.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/dll-hijacking.yaml -silent"

echo "[hex] V-115: DLL hijacking → $TARGET"
...
python3 - \
    ...
    "V-115" "DLL hijacking" "Cloud, Client, Availability and LLM" "nuclei" \
