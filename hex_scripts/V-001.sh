#!/usr/bin/env bash
# V-001.sh — Sensitive data exposure via OSINT
# Category: Discovery, Web and API
# Tool: theharvester
# Usage: bash V-001.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-001.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-001.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-001.sh <session> <target>}"
...
CMD="theharvester -d $TARGET -b all -l 500"

echo "[hex] V-001: Sensitive data exposure via OSINT → $TARGET"
...
python3 - \
    ...
    "V-001" "Sensitive data exposure via OSINT" "Discovery, Web and API" "theharvester" \
