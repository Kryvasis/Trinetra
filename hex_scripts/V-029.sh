...
# Tool: sqlmap
# Usage: bash V-029.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-029.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-029.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-029.sh <session> <target>}"
...
CMD="sqlmap -u http://$TARGET --batch --level=2 --risk=1 --crawl=2 --forms"

echo "[hex] V-029: SQLi → $TARGET"
...
python3 - \
    ...
    "V-029" "SQLi" "Discovery, Web and API" "sqlmap" \
