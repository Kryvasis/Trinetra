#!/usr/bin/env bash
# V-107.sh — Overly permissive IAM / Policy misconfiguration
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-107.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-107.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-107.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-107.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t misconfiguration/aws/ -silent"

echo "[hex] V-107: Overly permissive IAM / Policy misconfiguration → $TARGET"
...
python3 - \
    ...
    "V-107" "Overly permissive IAM / Policy misconfiguration" "Cloud, Client, Availability and LLM" "nuclei" \
