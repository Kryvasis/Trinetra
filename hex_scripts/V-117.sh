#!/usr/bin/env bash
# V-117.sh — AML / EDR bypass
# Category: Cloud, Client, Availability and LLM
# Tool: nuclei
# Usage: bash V-117.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-117.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-117.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-117.sh <session> <target>}"
...
CMD="nuclei -u http://$TARGET -t vulnerabilities/generic/edr-bypass.yaml -silent"

echo "[hex] V-117: AML / EDR bypass → $TARGET"
...
python3 - \
    ...
    "V-117" "AML / EDR bypass" "Cloud, Client, Availability and LLM" "nuclei" \
