#!/usr/bin/env bash
# V-090.sh — LLMNR Poisoning
# Category: Host, Software and internal architecture
# Tool: responder
# Usage: bash V-090.sh <SESSION> <TARGET>
# Place at: ~/Desktop/trinetra/scripts/hexstrike/V-090.sh
set -euo pipefail

SESSION="${1:?ERROR: usage: bash V-090.sh <session> <target>}"
TARGET="${2:?ERROR: usage: bash V-090.sh <session> <target>}"
...
echo "[hex] V-090: LLMNR Poisoning → $TARGET"
...
python3 - \
    ...
    "V-090" "LLMNR Poisoning" "Host, Software and internal architecture" "responder" \
