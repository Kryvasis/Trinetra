...
The workspace masthead is in normal flow, not fixed. Masthead and content share a centered maximum of 1440px with 40px side gutters. Navigation wraps. Overview uses an asymmetric .9fr/1fr/1.2fr metric layout, followed by linked workflow stages and a device preview. Common panels use 24px padding; overview panels use 26px. Nested panels flatten into separators rather than stacked card shells. Natural document scrolling remains canonical; tables own horizontal scrolling. The workspace reserves 116px below route content so the optional Runtime launcher never covers final-page controls or evidence rows.
...
### Runtime evidence console

The shared runtime console is optional operational instrumentation for collection and ingestion. A compact fixed Runtime launcher follows the active assessment across workspace routes and reports its textual state plus confirmed event count without entering the page layout. It is closed by default. User activation opens a right-side, non-modal evidence drawer, leaving the underlying workspace usable. The drawer follows a restrained Ubuntu/Kali shell convention: one title bar and timestamped prompt, command and output pairs. It does not repeat operation metadata, job gutters, branch glyphs, banners or buffer statistics. Every visible entry represents a backend-recorded stage. Commands are sanitized representations of the real API, collection and Java-helper operations. Raw configuration lines, credentials, tokens, temporary paths and secret values never enter the trace. Never add invented PIDs, network traffic, threat claims or decorative terminal activity merely to make this surface appear busier.

The drawer has explicit Close, pause/resume and copy-trace controls. Close and Escape dismiss it and return focus to the Runtime launcher; this documented non-modal variant does not trap focus or isolate the background. Pause stops refresh without stopping the assessment. Copy exports only the sanitized visible trace. Empty, live, success, partial, cancelled, error and unavailable states use text in addition to semantic color. The trace is bounded, session-scoped, atomically persisted and treated as operational metadata rather than hash-chain evidence.

### Motion

Workspace routes arrive over 220ms with `cubic-bezier(.16, 1, .3, 1)`, fading from .7 opacity and rising 5px. The requested runtime drawer enters once from the right over 240ms; new runtime lines arrive once over 160ms with a 3px vertical settle. This event-bound motion makes newly confirmed backend work legible and never loops. Button/navigation color/background/border and workflow-hover transitions take 160ms. Inputs retain the inherited 200ms border/background transition. Progress values update without animating layout width; zero is rendered as zero, without a decorative minimum fill. No perpetual workspace animation is introduced. Reduced motion removes workspace animations and transitions and resets scroll behavior to auto. Forced-color mode adds system-color panel borders and a highlighted current-navigation border.
