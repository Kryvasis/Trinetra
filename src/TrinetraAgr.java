...
public class TrinetraAgr {
    ...
    static {
        ...
        // ── Expanded P0-1/R2: 13 additional V-codes to reach 28 (pure JSON manifest, no Java wiring needed; curated steps keep 28/28 coverage) ──
        REMEDIATION.put("V-004", "1. `configure terminal`. 2. Harden DNS: `no ip domain-lookup` (or restrict) → `ip name-server 10.0.0.53` trusted only → `ip dns server` disable if unused. 3. Enable DNSSEC: `ip dnssec` (Juniper: `set system name-server 10.0.0.53` + `set system domain-name` trusted). 4. Verify: `show hosts` / `show run | include name-server` + test `dig +dnssec`.");
        REMEDIATION.put("V-010", "1. `configure terminal`. 2. Enforce HTTPS: `ip http secure-server` + `no ip http server`. 3. Add HSTS on upstream proxy/WAF: `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`. 4. Verify: `curl -I https://<device>` shows HSTS header; `show ip http server status`.");
        REMEDIATION.put("V-056", "1. Identify token gen: `show running-config | include session|token|cookie`. 2. Enforce entropy: `aaa new-model` + `crypto key generate rsa modulus 2048` + configure `session token random` / `crypto random` provider. 3. Add Secure/HttpOnly/SameSite on web mgmt (or proxy). 4. Verify: capture token, check `python3 -c 'import math, collections; print(entropy)'` >=128 bits, no reuse.");
        REMEDIATION.put("V-059", "1. Audit storage: `dir flash:` / `show flash` (Juniper: `file list /var/log`). 2. Remove plaintext: `no service password-encryption` → use `enable secret` + `service password-encryption` + `key config-key`. 3. Enable at-rest: `crypto key encrypt write` + `vtp mode transparent` encrypted, SONiC: `sonic-cfggen` encrypted. 4. Verify: `more flash:config.text` shows encrypted values only.");
        REMEDIATION.put("V-073", "1. Audit: `show running-config | include file|dir` + `dir flash:` (Linux: `ls -l /bootflash`). 2. Fix perms: `chmod 600 flash:config.text` (IOS: `file privilege 15`) / Juniper: `set system file permissions`. 3. Remove world-writable: `no file privilege 2` + tighten `archive` path. 4. Verify: `show privilege` / `ls -l` shows 600/700 only.");
        REMEDIATION.put("V-074", "1. Audit crons: `show run | include kron|cron` (Juniper: `show system cron`; Linux: `crontab -l` + `ls -l /etc/cron.*`). 2. Remove writable: `no kron policy-list` / `chmod 600 /etc/cron.*` + `chown root:root`. 3. Restrict: `kron occurrence` only root, `no service cron` if unused. 4. Verify: `show kron schedule` / `crontab -l` no low-priv writable entry.");
        REMEDIATION.put("V-088", "1. Generate SBOM: `syft <image> -o cyclonedx-json > sbom.json` (or `syft packages dir:/flash`). 2. Attach: store `sbom.json` in `sessions/<s>/artifacts/` + `crypto pki` sign. 3. Validate: `grype sbom:./sbom.json` + `syft sbom.json -o table`. 4. CI gate: add `syft` to pipeline, fail on incomplete `completeness <100%`.");
        REMEDIATION.put("V-104", "1. Disable DTP: `configure terminal` → `interface range Gi0/1-48` → `switchport mode access` → `switchport nonegotiate` → `no switchport trunk encapsulation`. 2. Prune VLANs: `switchport trunk allowed vlan 10,20` only (Juniper: `set vlans vlan10 vlan-id 10` + `delete protocols lldp`). 3. Harden native: `vlan dot1q tag native`. 4. Verify: `show interfaces trunk` / `show dtp interface`.");
        REMEDIATION.put("V-108", "1. Audit SG: `aws ec2 describe-security-groups --group-ids sg-xxxx` (Azure: `az network nsg show`). 2. Revoke open: `aws ec2 revoke-security-group-ingress --group-id sg-xxxx --protocol tcp --port 22 --cidr 0.0.0.0/0` (repeat 23,80,443,3389,5432). 3. Restrict: `aws ec2 authorize-security-group-ingress --group-id sg-xxxx --protocol tcp --port 22 --cidr 10.0.0.0/16`. 4. Verify: `aws ec2 describe-security-groups` shows no 0.0.0.0/0 on sensitive ports + NACL deny.");
        REMEDIATION.put("V-110", "1. Run CSPM: `prowler aws --severity critical,high` (or `steampipe check all`). 2. Fix failures: per-finding e.g. `aws s3api put-bucket-encryption` / `aws iam update-account-password-policy`. 3. Re-scan: `prowler --checks-file` until 0 fails. 4. Gate: CI `prowler --fail-on-findings` blocks deploy.");
        REMEDIATION.put("V-113", "1. Scan: `trufflehog git --since-commit HEAD` + `grep -r \"BEGIN PRIVATE\" .` . 2. Revoke: rotate exposed secret in vault (`aws secretsmanager rotate-secret`). 3. Purge: `git filter-repo --strip-blobs-with-ids` + `no logging` of secrets in pipeline (`set +x` + mask). 4. Verify: `trufflehog filesystem --no-verification` 0 hits + `show run | include secret` encrypted only.");
        REMEDIATION.put("V-118", "1. Check: `show run | include jdwp` + `nmap -p 8000,1044 <target>` (Linux: `ss -tlnp | grep jdwp`). 2. Disable: `no debug jdwp` / `no service jdwp` + remove `-agentlib:jdwp` from startup (Juniper: `delete system services jdwp`). 3. Firewall: `access-list 100 deny tcp any any eq 8000` → `interface mgmt0 ip access-group 100 in`. 4. Verify: `nmap -p 8000` closed + `show run | include jdwp` empty.");
        REMEDIATION.put("V-145", "1. Check: `show version` + `checksec --file=$(which iosd)` / `paxtest` (Linux: `sysctl kernel.exec-shield`). 2. Enable NX: `no no ip cef` + `service exec-shield` (Juniper: `set system allow-exec off`; Linux: `sysctl -w kernel.exec-shield=1` + `checksec` shows NX enabled). 3. Harden: `service password-encryption` + `stack-protector-strong` compile flag. 4. Verify: `checksec` shows NX/PIE/RELRO/Canary true; `paxtest` kiddie 0 killed.");
    }
