...
public class TrinetraAgr {
    ...
    // These are review-required guidance templates, not executable change plans.
    // Exact syntax and availability vary by model, release, feature set, and
    // management plane. Cortex never executes them and never labels them as
    // vendor-certified.
    //
    ...
    static {
        ...
        REMEDIATION.put("V-004", "Collect resolver configuration and live DNSSEC validation evidence. Confirm the device model and release support, then restrict resolvers to approved addresses using the applicable vendor guide. Re-run live resolution tests before closing the finding.");
        REMEDIATION.put("V-010", "Verify the management endpoint live, identify which device or reverse proxy owns the HTTP headers, and enable HTTPS/HSTS only through that component's release-specific documentation. Test redirects, certificate trust, and administrative access before and after the approved change.");
        REMEDIATION.put("V-056", "Use an authenticated application test to measure token unpredictability, reuse, expiry, and cookie attributes. Remediate in the component that issues the token; configuration text alone is insufficient evidence.");
        REMEDIATION.put("V-059", "Inventory secret-bearing files and storage controls with authorized host/device evidence. Move secrets to the approved secret store and enable supported at-rest protection; verify recovery and avoid displaying secret values in Cortex evidence.");
        REMEDIATION.put("V-073", "Collect filesystem or platform authorization evidence, identify the file owner and required service account, then apply least privilege using the platform's supported mechanism. Verify service startup and rollback before closure.");
        REMEDIATION.put("V-074", "Enumerate scheduled-task definitions and their owners from the live platform. Remove unneeded jobs and restrict modification rights through the native scheduler/OS controls after change approval; re-enumerate to verify.");
        REMEDIATION.put("V-088", "Generate an SBOM from an authoritative image or software inventory, validate component identifiers, store the artifact with the assessment, and gate deployment on an agreed completeness policy. A device configuration alone cannot establish SBOM coverage.");
        REMEDIATION.put("V-104", "On the specifically affected switch ports, replace dynamic trunk negotiation with an explicitly approved access or trunk mode and restrict allowed VLANs. Use the exact model/release guide, preserve management reachability, and verify operational trunk state after the change.");
        ...
        REMEDIATION.put("V-110", "Run an approved CSPM assessment with read-only credentials, triage each result against asset context, remediate through reviewed infrastructure-as-code, and re-scan. Do not infer cloud posture from a device configuration export.");
        REMEDIATION.put("V-113", "If an exposed secret is confirmed, revoke or rotate it first, preserve incident evidence, remove it from current and historical sources through the repository owner's approved process, and run a secret scan to verify. Never place secret values in Cortex.");
        REMEDIATION.put("V-118", "Confirm the debug interface with authorized live port/process evidence, remove the debug listener from the owning service configuration, restrict network reachability, restart through the change process, and verify the port is closed.");
        REMEDIATION.put("V-145", "Collect binary/platform hardening evidence using a tool supported for the target operating system. Upgrade or rebuild the affected component with vendor-supported execution protections; configuration text alone cannot establish NX/DEP status.");
    }
    ...
    public static String getRemediation(String vCode, String vendor, String osVersion) {
        ...
        if (v.contains("forti") || v.equals("fortios")) {
            ...
            if (base.contains("configure terminal") || base.contains("line vty"))
                return prefix + "Review-required outcome: use the control intent below, then select syntax from the exact JUNOS release documentation. Do not translate IOS commands mechanically. " + base;
